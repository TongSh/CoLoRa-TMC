...
%   including£ºposition, length, amplitude, frequency
    ...
    properties
        peak_ratio  % height of the latter peak devided by the former one
    ...
    methods
        function obj = csymbol(freq, amp, len, peak_ratio)
            % constractor
            ...
            obj.freq = freq;
            obj.bin = (BW - freq)/BW * 2^SF;
            obj.len = len;
            obj.amp = amp;
            obj.chirp_n = nsamp;
            obj.pkt_id = 0;
            obj.peak_ratio = peak_ratio;
        end
        ...
        function show(obj)
            ...
            fprintf('symbol amplitude = %.2f, length = %d, ratio = %d, belong = %d\n', obj.amp, round(obj.len), obj.peak_ratio, obj.pkt_id);
%             if obj.ahead
...
%             end
            fprintf(fid,'\n%d,%.1f,%.3f,%.2f,%d,%d', wid, obj.bin, obj.peak_ratio, obj.amp*100,belong,value);
            fclose(fid);
        end
