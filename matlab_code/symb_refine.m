function [dout,sym] = symb_refine(near_prev,seg_length, seg_ampl, peak_freq, org_sig)
    ...
    for pending_phase = (0:19)/20*2*pi
        sig = seg_ampl * symb_gen_phase(2^SF * (1 - peak_freq/BW),pending_phase,rphase_2);
        if near_prev
    ...
    for pending_phase = (0:19)/20*2*pi
        sig = seg_ampl * symb_gen_phase(2^SF * (1 - peak_freq/BW),rphase_1,pending_phase);
        if near_prev
            sig(round(seg_length+1):end) = 0;
        else
            sig(1:round(end-seg_length)) = 0;
...
%             continue;
%         end
%         sig = seg_ampl * symb_gen_phase(2^SF * (1 - i/BW),rphase_1,rphase_2);
%         if near_prev
%             sig(round(seg_length+1):end) = 0;
%         else
%             sig(1:round(end-seg_length)) = 0;
...
% 
%     for pending_phase = (0:19)/20*2*pi
%         sig = seg_ampl * symb_gen_phase(2^SF * (1 - r_freq/BW),pending_phase,rphase_2);
%         if near_prev
%             sig(round(seg_length+1):end) = 0;
%         else
%             sig(1:round(end-seg_length)) = 0;
...
% 
%     for pending_phase = (0:19)/20*2*pi
%         sig = seg_ampl * symb_gen_phase(2^SF * (1 - r_freq/BW),rphase_1,pending_phase);
%         if near_prev
%             sig(round(seg_length+1):end) = 0;
%         else
%             sig(1:round(end-seg_length)) = 0;
...
%     end
    ...
    sym = csymbol(r_freq, r_ampl, r_length, Inf);
%     figure;
%         subplot(3,1,1);
%             plot(real(org_sig));
%         subplot(3,1,2);
